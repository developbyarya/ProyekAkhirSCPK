...
import pandas as pd
from helper import split_empasis, count_matches, budget_score
...
with st.sidebar:
    ...
    df_preprocessed['control'] = (df_preprocessed['control'] == public_or_private_pref.lower()).astype(int)
    ...
    df_preprocessed['academic-emphasis_match'] = df_preprocessed['academic-emphasis'].apply(lambda x: count_matches(x, academic_emphasis_pref))
    ...
    df_preprocessed['state'] = df_preprocessed['state'].isin(state_pref).astype(int)
    ...
    df_preprocessed['within_budget'] = df['expenses'].between(budget_pref[0], budget_pref[1]).astype(int)
    df_preprocessed['budget_score'] = df_preprocessed['expenses'].apply(lambda x: budget_score(x, budget_pref[0], budget_pref[1]))


    # Wight section
    st.header("Bobot Kriteria", divider=True)
    # weight for state (1-5)
    weight_state = st.slider("Bobot Lokasi", min_value=0, max_value=5, value=2, step=1, key="weight_state")
    # weight for control (private / public university) (1-5)
    weight_control = st.slider("Bobot Swasta/Negri", min_value=0, max_value=5, value=2, step=1, key="weight_control")
    # weight for academic emphasis (1-5)
    weight_academic_emphasis = st.slider("Bobot Emphasis Akademik", min_value=0, max_value=5, value=2, step=1, key="weight_academic_emphasis")
    # bobot budget (1-5)
    weight_budget = st.slider("Bobot Budget", min_value=0, max_value=5, value=2, step=1, key="weight_budget")
    # weight untuk SAT skor (1-5)
    weight_sat = st.slider("Bobot SAT Skor", min_value=0, max_value=5, value=2, step=1, key="weight_sat")
    # weight untuk percent - financial aid (1-5)
    weight_financial_aid = st.slider("Bobot Financial Aid", min_value=0, max_value=5, value=2, step=1, key="weight_financial_aid")
    # weight untuk accaptance rate(1-5)
    weight_acceptance_rate = st.slider("Bobot % diterima", min_value=0, max_value=5, value=2, step=1, key="weight_acceptance_rate")
    # weight for popularity (1-5)
    weight_popularity = st.slider("Bobot Popularitas", min_value=0, max_value=5, value=2, step=1, key="weight_popularity")
    # bobot untuk kualitas akademik (1-5)
    weight_academic_quality = st.slider("Bobot Kualitas Akademik", min_value=0, max_value=5, value=2, step=1, key="weight_academic_quality")
    # bobot untuk kualitas sosial (1-5)
    weight_social_quality = st.slider("Bobot Kualitas Sosial", min_value=0, max_value=5, value=2, step=1, key="weight_social_quality")
